...
#if !defined(BITCOIN_TX) // TODO: This is an workaround. sodatoken-tx does not have access to chain state so no replay protection is possible
...
#if defined(BITCOIN_TX) // sodatoken-tx does not have access to chain state so no replay protection is possible
